...
Example: j$.mobile.changePage("#page_extra3", { transition: "none", role: "page" });
...
{
    try{
    common.PutToDebugLog(4, "EVENT, _extra3: onCreate");

    j$('#extra3_menu_ul').on('click', 'li', function(event)
    {
        MenuItemSelected(j$(this).attr('id'));
    });
    j$("#btn_extra3_menu").on("click", function() { CreateOptionsMenu('#extra3_menu_ul'); });
    
    // set page title
    j$("#extra3_title").html('Page Extra 3');
        
    } catch(err) { common.PutToDebugLogException(2, "_extra3: onCreate", err); }
}
...
{
    try{
    common.PutToDebugLog(4, "EVENT, _extra3: onStart");

    // set back button text
    j$('#extra3_btnback').html('< Go back');
    
    MeasureExtra3();
    PopulateData();
    
    } catch(err) { common.PutToDebugLogException(2, "_extra3: onStart", err); }
}

/** resolve window height size/resize changes */
function MeasureExtra3()
{
    j$('#page_extra3').css('min-height', 'auto'); // must be set when softphone is skin in div

    var contentHeight = common.GetDeviceHeight() - j$("#extra3_header").height() - 3;
    j$("#page_extra3_content").height(contentHeight);
}
...
function PopulateData()
{
    ...
    j$("#page_extra3_content").html(pageContent);

    } catch(err) { common.PutToDebugLogException(2, "_extra3: PopulateData", err); }
}
...
{
    try{
    if ( common.isNull(menuId) || menuId.lenght < 1 ) { common.PutToDebugLog(2, "ERROR, _extra3: CreateOptionsMenu menuid null"); return; }
    if (j$(menuId).length <= 0) { common.PutToDebugLog(2, "ERROR, _extra3: CreateOptionsMenu can't get reference to Menu"); return; }
...
// add menu items
    ...
    } catch(err) { common.PutToDebugLogException(2, "_extra3: CreateOptionsMenu", err); }
}
...
function MenuItemSelected(itemid)
{
    ...
    j$( '#extra3_menu' ).on( 'popupafterclose', function( event )
    {
        j$( '#extra3_menu' ).off( 'popupafterclose' );
    ...
    } catch(err) { common.PutToDebugLogException(2, "_extra3: MenuItemSelected", err); }
}
...
{
    try{
    common.PutToDebugLog(4, "EVENT, _extra3: onStop");

    // clear page content
    j$("#page_extra_content").html('');
    
    } catch(err) { common.PutToDebugLogException(2, "_extra3: onStop", err); }
}
