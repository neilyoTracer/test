...
Example: j$.mobile.changePage("#page_extra5", { transition: "none", role: "page" });
...
{
    try{
    common.PutToDebugLog(4, "EVENT, _extra5: onCreate");

    j$('#extra5_menu_ul').on('click', 'li', function(event)
    {
        MenuItemSelected(j$(this).attr('id'));
    });
    j$("#btn_extra5_menu").on("click", function() { CreateOptionsMenu('#extra5_menu_ul'); });
    
    // set page title
    j$("#extra5_title").html('Page Extra 5');
        
    } catch(err) { common.PutToDebugLogException(2, "_extra5: onCreate", err); }
}
...
{
    try{
    common.PutToDebugLog(4, "EVENT, _extra5: onStart");

    // set back button text
    j$('#extra5_btnback').html('< Go back');
    
    MeasureExtra5();
    PopulateData();
    
    } catch(err) { common.PutToDebugLogException(2, "_extra5: onStart", err); }
}

/** resolve window height size/resize changes */
function MeasureExtra5()
{
    j$('#page_extra5').css('min-height', 'auto'); // must be set when softphone is skin in div

    var contentHeight = common.GetDeviceHeight() - j$("#extra5_header").height() - 3;
    j$("#page_extra5_content").height(contentHeight);
}
...
function PopulateData()
{
    ...
    j$("#page_extra5_content").html(pageContent);

    } catch(err) { common.PutToDebugLogException(2, "_extra5: PopulateData", err); }
}
...
{
    try{
    if ( common.isNull(menuId) || menuId.lenght < 1 ) { common.PutToDebugLog(2, "ERROR, _extra5: CreateOptionsMenu menuid null"); return; }
    if (j$(menuId).length <= 0) { common.PutToDebugLog(2, "ERROR, _extra5: CreateOptionsMenu can't get reference to Menu"); return; }
...
// add menu items
    ...
    } catch(err) { common.PutToDebugLogException(2, "_extra5: CreateOptionsMenu", err); }
}
...
function MenuItemSelected(itemid)
{
    ...
    j$( '#extra5_menu' ).on( 'popupafterclose', function( event )
    {
        j$( '#extra5_menu' ).off( 'popupafterclose' );
    ...
    } catch(err) { common.PutToDebugLogException(2, "_extra5: MenuItemSelected", err); }
}
...
{
    try{
    common.PutToDebugLog(4, "EVENT, _extra5: onStop");

    // clear page content
    j$("#page_extra_content").html('');
    
    } catch(err) { common.PutToDebugLogException(2, "_extra5: onStop", err); }
}
